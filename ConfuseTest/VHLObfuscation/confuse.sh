...
HEAD_FILE="$PROJECT_DIR/VHLObfuscation/codeObfuscation.h"
...
grep -h -r -I  "^[-+]" $CONFUSE_FILE  --include '*.[mh]' |sed "s/[+-]//g"|sed "s/[();,: *\^\/\{]/ /g"|sed "s/[ ]*</</"| sed "/^[ ]*IBAction/d"|awk '{split($0,b," "); print b[2]; }'| sort|uniq |sed "/^$/d"|sed -n "/^$MOTHOD_MARK/p" >$STRING_SYMBOL_FILE
